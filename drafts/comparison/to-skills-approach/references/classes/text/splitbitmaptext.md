...
_extends `AbstractSplitText<BitmapText>`_

```ts
constructor(config: SplitBitmapTextOptions): SplitBitmapText
```

**Properties:**

- `defaultOptions: Partial<SplitBitmapTextOptions>` — Default configuration options for SplitBitmapText instances.
- `chars: BitmapText[]` — Individual character segments of the text.
- `words: Container<ContainerChild>[]` — Word segments of the text, each containing one or more characters.
- `lines: Container<ContainerChild>[]` — Line segments of the text, each containing one or more words.
- `_updateFlags: number`
...
- `isCachedAsTexture: boolean` — Whether this container is currently cached as a texture.
  **Methods:**
- `from(text: BitmapText, options?: Omit<SplitBitmapTextOptions, "text" | "style">): SplitBitmapText` — Creates a SplitBitmapText instance from an existing text object.
...
- `split(): void` — Splits the text into lines, words, and characters.
  Call this manually when autoSplit is false.
- `styleChanged(): void` — Used to notify the text that the style has changed.
  This will re-split the text and re-apply the style.
- `destroy(options?: DestroyOptions): void` — Destroys the SplitText instance and all its resources.
