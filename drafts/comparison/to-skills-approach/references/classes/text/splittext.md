...
_extends `AbstractSplitText<Text>`_

```ts
constructor(config: SplitTextOptions): SplitText
```

**Properties:**

- `defaultOptions: Partial<SplitTextOptions>` — Default configuration options for SplitText instances.
- `chars: Text[]` — Individual character segments of the text.
...
- `isCachedAsTexture: boolean` — Whether this container is currently cached as a texture.
  **Methods:**
- `from(text: Text, options?: Omit<SplitTextOptions, "text" | "style">): SplitText` — Creates a SplitText instance from an existing text object.
