...
_extends `ViewContainer<NineSliceSpriteGpuData>`_
_implements `View`_

```ts
constructor(options: Texture<TextureSource<any>> | NineSliceSpriteOptions): NineSliceSprite
```

**Properties:**

- `defaultOptions: NineSliceSpriteOptions` — The default options used to override initial values of any options passed in the constructor.
...
- `setSize(value: number | Optional<Size, "height">, height?: number): void` — Sets the size of the NineSliceSprite to the specified width and height.
...
- `getSize(out?: Size): Size` — Retrieves the size of the NineSliceSprite as a [Size]Size object.
