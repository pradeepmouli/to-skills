...
- `removeChildren(beginIndex?: number, endIndex?: number): C[]` — Removes all children from this container that are within the begin and end indexes.
...
- `setChildIndex(child: C, index: number): void` — Changes the position of an existing child in the container.
- `getChildIndex(child: C): number` — Returns the index position of a child Container instance.
