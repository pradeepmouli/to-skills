...
```ts
constructor(frames: AnimatedSpriteFrames, autoUpdate?: boolean): AnimatedSprite
```

**Properties:**

- `animationSpeed: number` — The speed that the AnimatedSprite will play at. Higher is faster, lower is slower.
- `loop: boolean` — Whether or not the animation repeats after playing.
...
- `updateAnchor: boolean` — Update anchor to [Texture's defaultAnchor]Texture#defaultAnchor when frame changes.
...
- `onComplete: () => void` (optional) — User-assigned function to call when an AnimatedSprite finishes playing.
...
- `onFrameChange: (currentFrame: number) => void` (optional) — User-assigned function to call when an AnimatedSprite changes which texture is being rendered.
...
- `onLoop: () => void` (optional) — User-assigned function to call when `loop` is true, and an AnimatedSprite is played and
...
- `isCachedAsTexture: boolean` — Whether this container is currently cached as a texture.
  **Methods:**
- `fromFrames(frames: string[]): AnimatedSprite` — A short hand way of creating an AnimatedSprite from an array of frame ids.
  Uses texture frames from the cache to create an animation sequence.
- `fromImages(images: string[]): AnimatedSprite` — A short hand way of creating an AnimatedSprite from an array of image urls.
  Each image will be used as a frame in the animation.
- `from(source: Texture<TextureSource<any>> | TextureSourceLike, skipCache: boolean): Sprite` — Creates a new sprite based on a source texture, image, video, or canvas element.
  This is a convenience method that automatically creates and manages textures.
- `mixin(source: Dict<any>): void` — Mixes all enumerable properties and methods from a source object to Container.
- `stop(): void` — Stops the animation playback and freezes the current frame.
  Does not reset the current frame or animation progress.
- `play(): void` — Starts or resumes the animation playback.
  If the animation was previously stopped, it will continue from where it left off.
- `gotoAndStop(frameNumber: number): void` — Stops the AnimatedSprite and sets it to a specific frame.
- `gotoAndPlay(frameNumber: number): void` — Goes to a specific frame and begins playing the AnimatedSprite from that point.
  Combines frame navigation and playback start in one operation.
- `update(ticker: Ticker): void` — Updates the object transform for rendering. This method handles animation timing, frame updates,
  and manages looping behavior.
- `destroy(options: DestroyOptions): void` — Stops the AnimatedSprite and destroys it.
...
- `getSize(out?: Size): Size` — Retrieves the size of the Sprite as a [Size]Size object based on the texture dimensions and scale.
  This is faster than getting width and height separately as it only calculates the bounds once.
- `setSize(value: number | Optional<Size, "height">, height?: number): void` — Sets the size of the Sprite to the specified width and height.
