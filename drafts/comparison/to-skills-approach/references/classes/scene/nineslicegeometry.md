...
```ts
constructor(options: NineSliceGeometryOptions): NineSliceGeometry
```

**Properties:**

- `defaultOptions: NineSliceGeometryOptions` — The default options for the NineSliceGeometry.
...
- `instanceCount: number` — the instance count of the geometry to draw
  **Methods:**
- `update(options: NineSliceGeometryOptions): void` — Updates the NineSliceGeometry with the options.
- `updatePositions(): void` — Updates the positions of the vertices.
- `updateUvs(): void` — Updates the UVs of the vertices.
