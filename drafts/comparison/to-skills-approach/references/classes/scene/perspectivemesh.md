...
> _extends `Mesh<PerspectivePlaneGeometry>`_

```ts
constructor(options: PerspectivePlaneOptions): PerspectiveMesh
```

**Properties:**

- `defaultOptions: PerspectivePlaneOptions` — Default options for creating a PerspectiveMesh instance.
...
- `setCorners(x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, x3: number, y3: number): void` — Sets the corners of the mesh to create a perspective transformation. The corners should be
