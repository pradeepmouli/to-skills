...
_extends `TextureSource<ICanvas>`_

```ts
constructor(options: CanvasSourceOptions): CanvasSource
```
...
- `resource: ICanvas` — the resource that will be uploaded to the GPU. This is where we get our pixels from
...
- `_sourceOrigin: string` — used internally to know where a texture came from. Usually assigned by the asset loader!
  **Methods:**
- `test(resource: any): resource is ICanvas`
- `resizeCanvas(): void`
