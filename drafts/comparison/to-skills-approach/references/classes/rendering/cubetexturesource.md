...
A TextureSource that represents a cube texture (6 faces).

Internally, WebGPU uses a 2D texture with 6 array layers and a `cube` view.
WebGL uses `TEXTURE_CUBE_MAP`.
_extends `TextureSource<CubeTextureFaces<TextureSource>>`_

```ts
constructor(options: CubeTextureSourceOptions): CubeTextureSource
```
...
- `faces: CubeTextureFaces<TextureSource<any>>` — The 6 face sources that make up this cube texture.
...
- `resource: CubeTextureFaces` — the resource that will be uploaded to the GPU. This is where we get our pixels from
...
Create a cube source from 6 already-created TextureSource instances:
