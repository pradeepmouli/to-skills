...
_extends `RenderTargetSystem<GlRenderTarget>`_

```ts
constructor(renderer: WebGLRenderer): GlRenderTargetSystem
```

**Properties:**

- `extension: { type: readonly [WebGLSystem]; name: "renderTarget" }`
- `adaptor: GlRenderTargetAdaptor` — a reference to the adaptor that interfaces with WebGL / WebGP
...
- `getGpuRenderTarget(renderTarget: RenderTarget): GlRenderTarget`
- `resetState(): void`
