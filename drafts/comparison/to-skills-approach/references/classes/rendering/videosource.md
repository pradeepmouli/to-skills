...
_extends `TextureSource<VideoResource>`_

```ts
constructor(options: VideoSourceOptions): VideoSource
```

**Properties:**

- `extension: ExtensionMetadata`
- `defaultOptions: VideoSourceOptions` — The default options for video sources.
- `MIME_TYPES: Dict<string>` — Map of video MIME types that can't be directly derived from file extensions.
...
- `isReady: boolean` — Whether or not the video is ready to play.
- `uploadMethodId: string` — The upload method for this texture.
...
- `resource: HTMLVideoElement` — the resource that will be uploaded to the GPU. This is where we get our pixels from
...
- `_sourceOrigin: string` — used internally to know where a texture came from. Usually assigned by the asset loader!
  **Methods:**
- `test(resource: any): resource is HTMLVideoElement`
- `load(): Promise<VideoSource>` — Start preloading the video resource.
- `destroy(): void` — Cleans up resources and event listeners associated with this texture.
