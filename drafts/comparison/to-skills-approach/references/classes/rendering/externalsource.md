...
_extends `TextureSource<GPUTexture | WebGLTexture>`_

```ts
constructor(__namedParameters: ExternalSourceOptions): ExternalSource
```
...
- `resource: GPUTexture | WebGLTexture` — the resource that will be uploaded to the GPU. This is where we get our pixels from
...
- `_sourceOrigin: string` — used internally to know where a texture came from. Usually assigned by the asset loader!
  **Methods:**
- `test(resource: unknown): resource is GPUTexture | WebGLTexture` — Test if a resource is a valid external GPU texture.
- `updateGPUTexture(gpuTexture: GPUTexture | WebGLTexture, width?: number, height?: number): void` — Update the external GPU texture reference.
