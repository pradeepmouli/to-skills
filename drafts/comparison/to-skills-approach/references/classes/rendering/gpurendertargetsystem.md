...
_extends `RenderTargetSystem<GpuRenderTarget>`_

```ts
constructor(renderer: WebGPURenderer): GpuRenderTargetSystem
```

**Properties:**

- `extension: { type: readonly [WebGPUSystem]; name: "renderTarget" }`
- `adaptor: GpuRenderTargetAdaptor` — a reference to the adaptor that interfaces with WebGL / WebGP
...
- `getGpuRenderTarget(renderTarget: RenderTarget): GpuRenderTarget`
- `resetState(): void`
