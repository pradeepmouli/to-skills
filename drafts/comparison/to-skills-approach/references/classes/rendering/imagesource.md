...
_extends `TextureSource<ImageResource>`_

```ts
constructor(options: TextureSourceOptions<ImageResource>): ImageSource
```
...
- `resource: ImageResource` — the resource that will be uploaded to the GPU. This is where we get our pixels from
